...
bool updateEnemy(Enemy * enemy, Map * map, Player * player){
    int deathanimation_tick = 64;
    if(enemy->status == DYING){
        enemy->death_animation_tick++;
        if(enemy->death_animation_tick > deathanimation_tick){
            enemy->status = DEAD;
            return true;
        }
...
void drawEnemy(Enemy * enemy, Point cam){
    ...
    if(enemy->status == ALIVE){
        int offset = 16 * (int)(enemy->animation_tick / 8); // frame
        if(enemy->animation_hit_tick > 0){
            offset += 16 * 8;
        }
        ...
        int flag = enemy->dir == RIGHT ? 1 : 0;
        int tint_red = enemy->knockback_CD > 0 ? 255 : 0;
        al_draw_tinted_scaled_rotated_bitmap_region(enemy->image, dying_offset, 16, 16, 16, al_map_rgb(tint_red, 255, 255),
            0, 0, dx, dy, TILE_SIZE / 16, TILE_SIZE / 16,
            0, flag);
        enemy->death_animation_tick++;
        if(enemy->death_animation_tick > deathanimation_tick){
...
void hitEnemy(Enemy * enemy, int damage, float angle){
    ...
    if(enemy->health<=0){
        enemy->health=0;
        enemy->status = DYING;
        printf("Enemy is dying. Health set to 0.\n");
    }
    ...
    */
    ...
    printf("Enemy status: %d, Knockback angle: %.2f\n", enemy->status, enemy->knockback_angle);
}
...
static bool isCollision(Point enemyCoord, Map* map){
    if( enemyCoord.x < 0 || 
        ...
        return true;
    int enemyXS = enemyCoord.x/TILE_SIZE;
    int enemyYS = enemyCoord.y/TILE_SIZE;
    int enemyXE = (enemyCoord.x + TILE_SIZE - 1) / TILE_SIZE;
    int enemyYE = (enemyCoord.y + TILE_SIZE - 1) / TILE_SIZE;
    
    if(!isWalkable(map->map[enemyYS][enemyXS]))return true;
    if(!isWalkable(map->map[enemyYS][enemyXE]))return true;
    if(!isWalkable(map->map[enemyYE][enemyXS]))return true;
    if(!isWalkable(map->map[enemyYE][enemyXE]))return true;
    /*
        [TODO HACKATHON 2-2] //just copy from player (hackathon 2-1)
