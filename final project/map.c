            ...
            switch(map->map[i][j]){
                case COIN:
                {
                    ...
                    if (map->coin_status[i][j] == APPEAR) {
                        offsetx = 16 * (map->coin_animation_tick / (64 / 8));
                    }
                    if (map->coin_status[i][j] == DISAPPEARING) {
                        map->dis_coin_status[i][j] = map->dis_coin_status[i][j] + 1;
                        offsety = 16;
                        offsetx = 16 * (map->dis_coin_status[i][j] / (64 / 8));
                        if (map->dis_coin_status[i][j] == 64) {
                            map->map[i][j] = FLOOR;
                        }
                    }
...
void update_map(Map * map, Point player_coord, int* total_coins){
    ...
    int i = (player_coord.x + 32) / TILE_SIZE;
    int j = (player_coord.y + 32) / TILE_SIZE;

    if(map->map[j][i] == COIN && map->coin_status[j][i] == APPEAR){
        map->coin_status[j][i] = DISAPPEARING;
