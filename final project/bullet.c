...
//    bullet->coord.y += 0;
    ...
    if(tile_x<0 || tile_y<0|| tile_x >= map-> row ||tile_y >= map->col||map->map[tile_x][tile_y] == WALL){ //sama yg kyk player isWalkable
        return true;
    }
    /*
        [TODO Hackathon 2-3] 
    ...
    */
    ...
    while(cur != NULL){
        ...
        if(bullet->coord.x >= enemyCoord.x && bullet->coord.x <= enemyCoord.x + TILE_SIZE && bullet->coord.y >= enemyCoord.y && bullet->coord.y <= enemyCoord.y + TILE_SIZE){
